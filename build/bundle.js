...
function search(q) {
  return new Promise((resolve, reject) => {
    ...
    _store2.default.dispatch((0, _searching.asyncSearching)(true));
    Promise.all(words.map(word => fetch(`https://api.spotify.com/v1/search?limit=50&q="${encodeURIComponent(word)}"&type=track`, {
      ...
      const crissCrossSorted = crissCross.map((list, index) => {
        ...
        return {
          previous: _lodash2.default.reverse(_lodash2.default.sortBy(list.previous, [item => item.name.length - (words[index - 1] || 0)]))[0] || null,
          self: _lodash2.default.sortBy(list.self, [item => item.name.length - words[index].length])[0] || null,
          next: _lodash2.default.reverse(_lodash2.default.sortBy(list.next, [item => item.name.length - (words[index + 1] || 0)]))[0] || null
        };
      });
